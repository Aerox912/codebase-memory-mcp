...
failures=0
expect() { # description log rc want
    local description="$1" log="$2" rc="$3" want="$4" got
    bash "$RUNNER" --verdict "$WORK/$log" "$rc" >/dev/null 2>&1
    got=$?
    if [ "$got" -ne "$want" ]; then
...
echo "VM verdict contract passed (8 cases: lost exit status, false green, partial runs)"
